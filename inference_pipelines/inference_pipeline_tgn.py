...
class InferencePipeLine:
    def __init__(self, config):
        ...
        self.first_module = GroupingNetworkModule()
        self.first_module.cuda()
        self.first_module.load_state_dict(
            torch.load(
                self.config["fps_model_info"]["load_ckpt_path"],
                weights_only=True,
            )
        )

        self.bdl_module = GroupingNetworkModule(
            stride=[1, 1],
            nsample=[36, 24],
            blocks=[2, 3],
            block_num=2,
        ...
        self.bdl_module.load_state_dict(
            torch.load(
                self.config["boundary_model_info"]["load_ckpt_path"],
                weights_only=True,
            )
        )
    ...
    def __call__(self, mesh, pca=False):
        ...
        vertices[:, :3] = (
            (vertices[:, :3] - np.min(vertices[:, 1]))
            / (np.max(vertices[:, 1]) - np.min(vertices[:, 1]))
        ) * self.scaler - self.shifter
        mesh.vertices = o3d.utility.Vector3dVector(vertices)
        org_feats = np.array(
            np.concatenate(
                [np.array(mesh.vertices), np.array(mesh.vertex_normals)], axis=1
            )
        )
        ...
        sampled_feats = gu.resample_pcd(
            vertices.copy(), 24000
        )  # TODO slow processing speed

        input_cuda_feats = (
            torch.from_numpy(np.array([sampled_feats.astype("float32")]))
            .cuda()
            .permute(0, 2, 1)
        )
        first_results = self.get_first_module_results(
            input_cuda_feats, self.first_module
        )
        (
            sampled_boundary_feats,
            sampled_boundary_seg_label,
            only_boundary_feats,
            only_boundary_seg_label,
        ) = self.get_boundary_sampled_feats(
            first_results["ins"]["full_ins_labeled_points"][:, 3],
            bdl_feats,
            sampled_feats,
            None,
        )
        input_cuda_bdl_feats = (
            torch.from_numpy(np.array([sampled_boundary_feats.astype("float32")]))
            .permute(0, 2, 1)
            .cuda()
        )
        sampled_boundary_seg_label = (
            torch.from_numpy(np.array([sampled_boundary_seg_label.astype(int)]))
            .permute(0, 2, 1)
            .cuda()
            - 1
        )
        bdl_results = self.get_second_module_results(
            input_cuda_bdl_feats, sampled_boundary_seg_label, self.bdl_module
        )
        first_xyz = first_results["ins"]["full_ins_labeled_points"][:, :3]
        first_ps_label = first_results["ins"]["full_ins_labeled_points"][:, 3].astype(
            int
        )
        first_sem_xyz = first_results["sem_1"]["full_labeled_points"][:, :3]
        first_sem_label = first_results["sem_1"]["full_labeled_points"][:, 3]
        bdl_xyz = bdl_results["ins"]["full_ins_labeled_points"][
            : only_boundary_feats.shape[0], :3
        ]
        bdl_ps_label = bdl_results["ins"]["full_ins_labeled_points"][
            : only_boundary_feats.shape[0], 3
        ].astype(int)
        ...
        first_ps_label_unique_except_zero = first_ps_label_unique_except_zero[
            first_ps_label_unique_except_zero != 0
        ]
        ins_label_center_points = np.array(
            [
                np.mean(first_xyz[first_ps_label == label], axis=0)
                for label in first_ps_label_unique_except_zero
            ]
        )
        ...
        pca_axis[2] = (
            pca_axis[2]
            if np.dot((teeth_mean - gin_mean).reshape(3), pca_axis[2]) > 0
            else -pca_axis[2]
        )
        if (
            np.where(first_sem_label == 1)[0].shape[0]
            + np.where(first_sem_label == 9)[0].shape[0]
            > 20
        ):
            centerpoints_of_11_12 = np.array(
                np.mean(
                    [
                        np.mean(first_sem_xyz[first_sem_label == 1], axis=0),
                        np.mean(first_sem_xyz[first_sem_label == 9], axis=0),
                    ],
                    axis=0,
                )
            )
            ...
            for i in range(2, 9):
                if np.where(first_sem_label == i)[0].shape[0] > 20:
                    centerpoints_of_11_12 = np.array(
                        np.mean(
                            [
                                np.mean(first_sem_xyz[first_sem_label == i], axis=0),
                                np.mean(ins_label_center_points, axis=0),
                            ],
                            axis=0,
                        )
                    )
        ...
        for ins_label in np.unique(first_ps_label):
            ...
            if sem_labels_in_cluster.shape[0] == 0:
                ...
                continue
            max_freq_first_cluster_label = np.argmax(
                np.bincount(sem_labels_in_cluster.astype(int))
            )
            if max_freq_first_cluster_label not in [1, 9]:
                if (
                    np.dot(
                        ins_points_center - centerpoints_of_11_12, checking_axis_vector
                    )
                    < 0
                ):
        ...
        for bdl_cluster_label in np.unique(bdl_ps_label):
            ...
            cluster_near_points = tree.query(
                bdl_cluster_points, k=1, return_distance=False
            )

            cluster_first_cluster_label = first_ps_label[
                cluster_near_points.reshape(-1)
            ]
            max_freq_first_cluster_label = np.argmax(
                np.bincount(cluster_first_cluster_label)
            )
            ...
            mod_bdl_ps_label[bdl_cluster_label == bdl_ps_label] = (
                max_freq_first_cluster_label
            )
        ...
        final_ins_labels = np.concatenate(
            [first_ps_label.reshape(-1, 1), mod_bdl_ps_label.reshape(-1, 1)], axis=0
        )
        final_ins_labels = final_ins_labels.astype(int)
        final_sem_labels = np.concatenate(
            [new_sem_labels.reshape(-1, 1), mod_bdl_sem_label.reshape(-1, 1)], axis=0
        )
        ...
        result_ins_labels = final_ins_labels.reshape(-1)[
            near_points.reshape(-1)
        ].reshape(-1, 1)
        result_sem_labels = final_sem_labels.reshape(-1)[
            near_points.reshape(-1)
        ].reshape(-1, 1)
    ...
    def get_first_module_results(self, feats, base_model):
        ...
        full_labeled_points_1 = np.concatenate(
            [org_xyz_cpu, whole_cls_1.reshape(-1, 1)], axis=1
        )
        ...
        full_masked_points_2 = np.concatenate(
            [org_xyz_cpu, whole_mask_2.reshape(-1, 1)], axis=1
        )
        ...
        moved_points_cpu = (
            org_xyz_cpu + gu.torch_to_numpy(output["offset_1"])[0, :3, :].T
        )
        fg_moved_points = moved_points_cpu[
            results["sem_2"]["full_masked_points"][:, 3] == 1, :
        ]

        fg_points_labels_ls = tu.get_clustering_labels(
            moved_points_cpu, results["sem_2"]["full_masked_points"][:, 3]
        )

        points_ins_labels = np.zeros(org_xyz_cpu.shape[0])
        points_ins_labels[:] = -1
        points_ins_labels[np.where(results["sem_2"]["full_masked_points"][:, 3])] = (
            fg_points_labels_ls
        )
        points_ins_labels += 1

        full_ins_labeled_points = np.concatenate(
            [org_xyz_cpu, points_ins_labels.reshape(-1, 1)], axis=1
        )
        results["ins"] = {}
    ...
    def get_second_module_results(self, feats, sampled_boundary_seg_label, base_model):
        ...
        points = feats
        with torch.no_grad():
            output = base_model(points, sampled_boundary_seg_label)
        results = {}
        ...
        if False:
            ...
            colors = (colors - colors.min(axis=0)) / (
                colors.max(axis=0) - colors.min(axis=0)
            )
        ...
        num_of_clusters.append(
            len(np.unique(gu.torch_to_numpy(sampled_boundary_seg_label))) - 1
        )
        cluster_centroids, cluster_centroids_labels, fg_points_labels_ls = (
            tu.clustering_points(
                [fg_moved_points], method="kmeans", num_of_clusters=num_of_clusters
            )
        )
    ...
    def get_boundary_sampled_feats(
        self, point_labels, org_feats, sampled_feats, sample_output_features
    ):
        ...
        label_ratio = label_counts[:, 0] / 40.0
        ...
        idx = np.random.permutation(bd_org_feat_cpu.shape[0])[
            : self.config["boundary_sampling_info"]["num_of_bdl_points"]
        ]
        bd_org_feat_cpu, bd_org_ps_label_cpu = (
            bd_org_feat_cpu[idx],
            bd_org_ps_label_cpu[idx],
        )
        non_bd_org_feat_cpu = org_feats[bd_labels != 1, :]
        non_bd_org_ps_label_cpu = ps_labels[bd_labels != 1, :]
        idx = np.random.permutation(non_bd_org_feat_cpu.shape[0])[
            : self.config["boundary_sampling_info"]["num_of_all_points"]
        ]
        non_bd_org_feat_cpu, non_bd_org_ps_label_cpu = (
            non_bd_org_feat_cpu[idx],
            non_bd_org_ps_label_cpu[idx],
        )

        results_feat_cpu = np.concatenate(
            [bd_org_feat_cpu, non_bd_org_feat_cpu], axis=0
        )
        results_label_cpu = np.concatenate(
            [bd_org_ps_label_cpu, non_bd_org_ps_label_cpu], axis=0
        )

        if sample_output_features is not None:
            sample_output_features = gu.torch_to_numpy(sample_output_features)[
                0, :, :
            ].T
            ...
            near_points_prop = near_points_prop / np.sum(
                near_points_prop, axis=1
            ).reshape(-1, 1)

            nn_features = sample_output_features[
                near_points_idxes
            ] * near_points_prop.reshape(24000, 3, 1)
