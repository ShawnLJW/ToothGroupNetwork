...
def batch_center_offset_loss(pred_offset, sample_xyz, gt_seg_label):
    ...
    for batch_idx in range(B):
        for tooth_num in range(0, 16):
            ...
            centroid_losses += torch.div(
                torch.sum(moved_dists), cls_sample_xyz.shape[1]
            )
            ...
            points_to_center_dir_norm = torch.norm(points_to_center_dir, dim=2).view(
                1, -1, 1
            )
            points_to_center_dir = torch.div(
                points_to_center_dir, points_to_center_dir_norm
            )

            cls_offset_dir = cls_offset_dir[cls_offset_norm.view(1, -1) > 0.0002]
            points_to_center_dir = points_to_center_dir[
                cls_offset_norm.view(1, -1) > 0.0002
            ]
...
def weighted_batch_center_offset_loss(
    pred_offset_1, pred_offset_2, sample_xyz, gt_seg_label
):
    """offset loss
    ...
    for batch_idx in range(B):
        for tooth_num in range(0, 16):
            ...
            weight_1[moved_dists_1 >= thr] = (
                weight_1[weight_1 >= thr] * 10 - thr * 10
            ) * 2 + 1
            ...
            centroid_losses += torch.div(
                torch.sum(moved_dists_2 * weight_1), cls_sample_xyz.shape[1]
            )
...
def second_distance_loss(pred_distance, pred_offset, sample_xyz, gt_seg_label):
    ...
    for i in range(0, 16):
        ...
        points_to_center_dir = centroid - cls_sample_xyz
        points_to_center_dir = torch.div(
            points_to_center_dir, torch.norm(points_to_center_dir)
        )

        dir_losses += -(
            torch.dot(cls_offset_dir.view(-1), points_to_center_dir.view(-1))
        )
...
def batch_chamfer_distance_loss(pred_offset, sample_xyz, gt_seg_label):
    ...
    for batch_idx in range(B):
        ...
        pred_ct_dists = square_distance(
            moved_points.unsqueeze(dim=0), b_centroids.unsqueeze(dim=0)
        )
...
def tooth_class_loss(cls_pred, gt_cls, cls_num, weight=None, label_smoothing=None):
    ...
    if label_smoothing is None:
        if weight is None:
            loss = (
                torch.nn.CrossEntropyLoss().type(torch.float).cuda()(cls_pred, gt_cls)
            )
        else:
            loss = torch.nn.CrossEntropyLoss(
                weight=torch.tensor(weight).type(torch.float).cuda()
            )(cls_pred, gt_cls)
    else:
...
def teeth_mask_loss(pred_mask, gt_seg_label):
    gt_bin_label = (
        make_teeth_mask(gt_seg_label).type(torch.float32).reshape(pred_mask.shape)
    )
...
def weighted_cls1_loss(pred_weight_1, pred_cls_1, gt_seg_label, weight):
    ...
    bce_1 = torch.nn.CrossEntropyLoss(weight=weight, reduction="none")(
        pred_cls_1, gt_seg_label
    )
...
def weighted_cls2_loss(pred_weight_1, pred_cls_2, gt_seg_label, weight):
    ...
    gt_seg_label = gt_seg_label.type(torch.long)
    gt_seg_label = gt_seg_label + 1
    bce_2 = torch.nn.CrossEntropyLoss(weight=weight, reduction="none")(
        pred_cls_2, gt_seg_label
    )
