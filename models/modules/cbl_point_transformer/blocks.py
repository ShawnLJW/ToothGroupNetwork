...
class PointTransformerLayer(nn.Module):
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        x_k, _ = pointops.query_and_group(
            self.nsample, p, p, x_k, None, o, o, with_xyz=True
        )  # (n, nsample, 3+c)
        x_v, _ = pointops.query_and_group(
            self.nsample, p, p, x_v, None, o, o, with_xyz=False
        )  # (n, nsample, c)
        ...
        s = self.share_planes
        x = (
            ((x_v + p_r).view(n, nsample, s, c // s) * w.unsqueeze(2)).sum(1).view(n, c)
        )  # v * A
        return x
...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            n_p = p[idx.long(), :]  # (m, 3)
            x, _ = pointops.query_and_group(
                self.nsample, p, n_p, x, None, o, n_o, with_xyz=True
            )  # (m, 3+c, nsample)
            x = self.relu(
...
class PointTransformerBlock(nn.Module):
    ...
    def forward(self, pxo):
        ...
        x = self.relu(
            self.bn2(self.transformer2([p, x, o]))
        )  # - seems like trans/convolute - bn - relu
...
class MLPbyOps(nn.Module):
    ...
    def __init__(self, ops, fdim, d_mid=None, d_out=None, **kwargs):
        ...
        for ops in ops_seq:
            ...
            linear = "linear" in ops or not ops.endswith(
                "mlp"
            )  # linear / linearbn / mlp2 to ends with linear
            ...
            def get_mlp(ops_list, din, dout, mlp_kwargs):
                ...
                if mlp_kwargs["activation"] == "relu":
                    ...
                    raise ValueError(
                        f"not support activation = " + mlp_kwargs["activation"]
                    )
                return ops_list

            for i in range(num_mlp - 1):
                ops_list = get_mlp(
                    ops_list, din=fdim, dout=d_mid, mlp_kwargs=mlp_kwargs
                )
                fdim = d_mid
