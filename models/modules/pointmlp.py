...
# group points based on their proximity
...
class PointMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(x_list)):
            ...
            # print('ggggg', gmp_list[0].size()) # torch.Size([3, 64, 1])
        global_context = self.gmp_map_end(torch.cat(gmp_list, dim=1)) # [b, gmp_dim, 1]
        # print('45666', global_context.size()) # torch.Size([3, 64, 1])
        ...
        # print('x before size: ', x.size())  #torch.Size([3, 17, 24000])
        # x = x.permute(0, 2, 1) #12 JUNE EDITED OUT
        # print('46555x size: ', x.size()) #torch.Size([3, 24000, 17])
...
class PointMLPFirstModule(torch.nn.Module):
    ...
    def forward(self, inputs, test=False):
        ...
        # print('inputs[0].size() isssss', inputs[0].size()) # torch.Size([3, 3, 24000])
        cls_pred = self.first_sem_model(inputs)
        # print('aaaaaa', cls_pred.size())
        outputs = {
            "cls_pred": cls_pred
        }
        return outputs
...
if __name__ == '__main__':
    ...
    #model = pointMLP(50)
    model = pointMLP_seg(num_classes=17, num_channels=3)
