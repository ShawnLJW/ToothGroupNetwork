...
from external_libs.pointnet2_utils.pointnet2_utils import (
    index_points,
    square_distance,
...
class LocalGrouper(nn.Module):
    def __init__(
        self, channel, groups, kneighbors, use_xyz=True, normalize="anchor", **kwargs
    ):
        """
        Give xyz[b,p,3] and fea[b,p,d], return new_xyz[b,g,3] and new_fea[b,g,k,d]
        :param groups: groups number
        :param kneighbors: k-neighbors
        :param kwargs: others
        """
...
class PointMLP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding = ConvBNReLU1D(
            num_channels, embed_dim, bias=bias, activation=activation
        )
        assert (
            len(pre_blocks)
            == len(k_neighbors)
    ...
    def forward(self, feat, gt_seg_label=None):
        # B, D, N
        xyz = feat[:, :3, :].permute(0, 2, 1)
        x = self.embedding(feat)  # B,D,N
        ...
        if gt_seg_label is not None:
            outputs["loss"] = tooth_class_loss(x, gt_seg_label, self.class_num)
        return outputs
